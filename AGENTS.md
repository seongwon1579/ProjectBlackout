...
- **기본 원칙**: 빌드, 에디터 실행, 실제 플레이 테스트는 사용자 책임으로 진행합니다.
- **AI 에이전트 행동 규칙**: 사용자의 명시적 요청 없이 `Visual Studio/MSBuild/UnrealBuildTool/에디터 실행` 등 무거운 검증 절차를 자동 수행하지 않습니다.
- **작업 종료 시 보고 방식**: 에이전트는 "빌드를 수행하지 않았음"을 명확히 적고, 사용자가 확인해야 할 대상(예: 수정 클래스, 예상 입력 경로, 블루프린트에 연결할 Input Action/Mapping Context)을 짧게 정리합니다.
- **예외**: 사용자가 "직접 빌드하지 못하니 대신 돌려달라"처럼 명확히 요청한 경우에만 빌드 또는 실행 검증을 시도할 수 있습니다.
