...
- **`ABlackoutBossCharacter`**: 보스는 풀링 대상이 아니므로 `OnReturnToPool`을 오버라이드하여 풀 반환 대신 `Destroy()` 처리. **어그로(타겟 선정)는 별도 컴포넌트를 두지 않고 `ABlackoutBossAIController`의 StateTree Evaluator(`FBSTEval_AggroTarget`)가 전담** — 03 다이어그램 참조.
