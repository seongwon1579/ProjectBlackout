...
- **공통 모듈**: `UBlackoutAggroEvaluator` / StateTree Aggro Evaluator, `UBOBossData` (페이즈·어그로 튜닝)
- **환경 오브젝트**: `ABOBreakablePillarActor` (Phase A/B 진행 중 Ravager 계열 히트로 순차 파괴되어 Phase C 회피 난이도 상승)
...
- **어그로(타겟 선정)**: `UBlackoutAggroEvaluator`와 StateTree Aggro Evaluator가 누적 피해 추적·감쇠·타겟 평가를 담당하고 Blackboard에 타겟을 퍼블리시합니다. `ABOShrewdBoss`의 `UBlackoutAggroComponent`는 현재 코드에 남아 있는 보조/디버그 경로입니다.
