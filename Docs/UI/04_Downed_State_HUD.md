...
| `Spectator` | 로컬 플레이어 `State.Dead == true` | 관전 HUD | 기본 전투 HUD, 다운/부활 프로그래스 바 | 관전 HUD 상세는 후속 확정 | 전멸 복귀 / 체크포인트 복귀 |
...
- **복구 조건**: 부활 성공으로 `State.Downed`와 `State.BeingRevived`가 해제되면 `Combat` 모드로 복귀합니다. 완전 사망으로 `State.Dead`가 적용되면 `Spectator` 모드로 전환하며, 관전 HUD 상세는 후속 문서에서 정의합니다.
