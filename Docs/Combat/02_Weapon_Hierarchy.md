...
- **투사체 풀링**: `ABOProjectile`은 `IBlackoutPoolableInterface` 구현. `UBlackoutPoolSubsystem::SpawnFromPool`로 획득.
  - `OnSpawnFromPool`: Collision/Movement 리셋, `DamageSpec` 주입, 나이아가라 이펙트 활성화 (`Activate`)
  - `OnReturnToPool`: Movement 정지, Collision 비활성화, `DamageSpec` 초기화, 나이아가라 이펙트 비활성화 (`Deactivate`)
