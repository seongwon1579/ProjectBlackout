...
| 투표권자 | 현재 접속 중인 모든 파티 참가자. Alive / Downed / Dead 모두 투표 가능 |
| 중복 투표 | PlayerState 단위로 1회만 인정 |
| 필요 표 수 | `floor(ConnectedPlayers / 2) + 1` |
| 투표 성공 | `HandlePartyWipe()`를 호출해 체크포인트 복귀 흐름 재사용 |
| 투표 초기화 | 전멸 복귀, 체크포인트 복귀, 매치 종료, 플레이어 이탈 시 초기화 |
| UI 표시 | 현재 표 수와 필요 표 수를 `GameState` 복제 값으로 표시 |
