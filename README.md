...
기술 스택 및 전체 구조
엔진: Unreal Engine 5.7 / C++
전투 프레임워크: GAS (Gameplay Ability System)
...
├─ AI/            ← 본인 담당 파트 (행동 트리·스테이트 트리·어그로·페이즈)
├─ GAS/           어빌리티/어트리뷰트/이펙트/큐
├─ Combat/        무기·전투 컴포넌트·히트박스
├─ Characters/    플레이어·보스·미니언 캐릭터
├─ Framework/     게임모드·매치메이킹·세션
├─ Animation/     애님 인스턴스·노티파이
├─ Pool/          오브젝트 풀링 서브시스템
└─ UI/            HUD·로비·매치 결과
<br>
담당 파트: AI 시스템
<!-- [ AI 시스템 다이어그램 자리 ] --> <!-- ![AI Architecture](docs/images/ai_architecture.png) -->

적 AI는 컨트롤러 → 행동 로직(BT/ST) → 어빌리티(GAS) 로 이어지는 구조로 설계했으며, 어그로·페이즈 판정 같은 공통 로직은 별도 평가기(Evaluator)로 분리해 재사용성을 높였습니다.
...
구분	사용 아키텍처	설계 의도
Ravager (근접 보스)	Behavior Tree	페이즈별로 트리를 교체하며 패턴 위주로 전개
Shrewd (원거리 보스)	State Tree	카이팅·텔레포트 등 상태 전환 중심
미니언 (Hollow / Wraith)	State Tree	가벼운 단독 구동, 서브 BT 없이 처리
<br>
1. 어그로 평가기 (Aggro Evaluator)

멀티플레이 PvE에서 "적이 누구를 노릴 것인가"를 결정하는 핵심 로직입니다. 단순 근접이 아니라 거리 · DPS · 저체력 을 가중 합산해 타겟을 산정합니다.

DPS 윈도우: 최근 일정 시간(기본 3초) 내에 준 데미지만 누적해 "지금 위협적인 플레이어"를 반영
가중치 분리: 거리·DPS·저체력 가중치를 에디터에서 조정 가능(EditAnywhere)
예외 처리: 사망/다운된 타겟은 후보에서 제외
BT / ST 공용: 두 아키텍처 모두에서 동일한 평가기를 재사용
<!-- [ 어그로 평가 흐름 이미지 자리 ] -->
cpp
...
보스 체력 구간에 따라 Phase 1 → 2 → 3 으로 행동을 전환합니다. 연출·공격 중 페이즈가 바뀌면 어색하기 때문에, 잠금 태그(Lock Tag) 를 두어 전환을 보류했다가 잠금 해제 시 반영하도록 설계했습니다.

특정 GameplayTag가 활성인 동안 페이즈 전환 보류(Pending)
잠금 해제 시 대기 중이던 페이즈를 적용
페이즈 변경은 델리게이트로 브로드캐스트 → BT 러너가 트리 교체
<!-- [ 페이즈 전환 다이어그램 자리 ] --> <br>
...
<!-- [ Ravager Behavior Tree 이미지 자리 ] --> <br>
...
<br> <!-- ======================================================================== 이미지 추가 안내 - docs/images/ 폴더를 만들어 이미지를 넣고 위 주석 자리의 경로를 교체하세요. - 플레이 영상은 유튜브 링크로 메인 썸네일에 연결하는 것을 권장합니다. ======================================================================== -->
